...
import { FaLightbulb, FaShieldAlt, FaHandsHelping, FaCheckCircle, FaHeadset, FaUsers } from "react-icons/fa"; // Importing more relevant icons

const values = [
  {
    icon: <FaLightbulb />, // Icon for Innovation (Lightbulb)
    title: "Innovation",
    description: "We foster a culture of continuous innovation to drive progress in healthcare.",
  },
  {
    icon: <FaShieldAlt />, // Icon for Integrity (Shield)
    title: "Integrity",
    description: "We operate with honesty, ethics, and transparency in all our interactions.",
  },
  {
  ...
  },
  {
    ...
    title: "Quality",
    description: "We uphold the highest standards in all our products and services.",
  },
  {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0">
                    {values.map((value, index) => (
                        <motion.div
                            ...
                            className={`p-6 border-[1px] border-gray-300 ${index < 3 ? "border-b-0 border-r-0 " : "" // Add bottom border for cards in first row
                        ...
                        >
                            ...
                            {value.title && (
                                <h3 className={`text-xl font-bold  ${index===2?"text-thLightBlue":"text-gray-800"}`}>{value.title}</h3>
                            )}
                            {value.subtitle && (
                                <p className={`mt-2 font-bold text-black ${index===2?"text-3xl":""}`}>{value.subtitle}</p>
                            )}
                            {value.description && (
